...
<div class="valuable-property-form">
    ...
    <?= $form->field($model, 'property_type')->dropDownList(ValuableProperty::$property_type) ?>

    <?= $form->field($model, 'name')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'cost')->textInput() ?>

    <?= $form->field($model, 'location')->dropDownList(ValuableProperty::$location,['onchange' => 'changeAdminValuablePropertyLocation();']) ?>
    ...
    <?$display = $model->location == 2 ? "block" : "none"?>

    <div style="display: <?=$display?>" class="location_block">

        <?= $form->field($model, 'org_name')->textInput(['maxlength' => true]) ?>

        <?= $form->field($model, 'dogovor_number')->textInput(['maxlength' => true]) ?>

        <?= $form->field($model, 'dogovor_date')->widget(DatePicker::classname(), [
            'options' => [
        ...
        <?= $form->field($uploadForm, "valuable_property[]")->fileInput(['multiple' => true])->label('Скан документа') ?>
    ...
    <?= $form->field($model, 'zalog_type')->dropDownList(ValuableProperty::$zalog_type) ?>
