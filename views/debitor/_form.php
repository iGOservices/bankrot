...
<div class="debitor-form">

    <?php $form = ActiveForm::begin(); ?>

    <?//= $form->field($model, 'ticket_id')->textInput() ?>

    <?= $form->field($model, 'group')->dropDownList(Debitor::$group,['onchange' => 'changeAdminDebitorStatment();']) ?>

    <div id="debitor_commitment">
        <?if($model->group == 1 || !$model->group):?>

            <?= $form->field($model, 'commitment')->dropDownList(Debitor::$commitment) ?>
    ...
    <?= $form->field($model, 'statment')->dropDownList(Debitor::$statment) ?>
    ...
    <?= $form->field($uploadForm, "debitor[]")->fileInput(['multiple' => true])->label('Скан документа') ?>
