...
<div class="razvod-form">
    ...
    <?= $form->field($uploadForm, "razvod[]")->fileInput(['multiple' => true])->label('Скан свидетельства о разводе') ?>

    <?
    $str = "";
    if($files = $model->getRazvodFiles()->all()){
        $str .= "<span>Скан свидетельства о разводе:</span><ul>";
    ...
    <?= $form->field($uploadForm, "property_division[]")->fileInput(['multiple' => true])->label('Соглашение о разделе имущества') ?>

    <?
    $str = "";
    if($files = $model->getPropertyDivisionFiles()->all()){
        $str .= "<span>Соглашение о разделе имущества:</span><ul>";
        foreach ($files as $file){
            $str.= "<li id=\"".$file->id."\">
                                <a href=\"".$file->getLink(true,'sp',$model->ticket_id)."\" target=\"_blank\">
                                    ".StringHelper::truncate($file->origin,30,'...').".".$file->ext."
